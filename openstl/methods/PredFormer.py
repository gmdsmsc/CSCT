...
class PredFormer(Base_method):
    ...
    def _predict(self, batch_x, label_x, label_y, batch_y=None, **kwargs):
        if self.args.aft_seq_length == self.args.pre_seq_length:
            ...
            pred_y = self.model(batch_x, label_x, label_y)
            ...
            for i in range(d):
                output = self.model(cur_seq, label_x, label_y)
                pred_y.append(output)

            if m != 0:
                output = self.model(cur_seq, label_x, label_y)
                pred_y.append(output[:, :m])
    ...
    def train_one_epoch(self, runner, train_loader, epoch, num_updates, eta=None, **kwargs):
        ...
        for batch_x, batch_y, label_x, label_y, subset_no in train_pbar:
            ...
            with self.amp_autocast():
                pred_y = self._predict(batch_x, label_x, label_y)
            ...
            if self.rank == 0:
                log_buffer = f'train loss: {loss.item():.4f} | data time: {data_time_m.avg:.4f}'
